...
def run():
    ...
    with torch.no_grad():
        for i in df.index:
            ...
            # Convert raw_hm to a PIL image for resizing
            raw_hm_image = Image.fromarray(raw_hm.astype(np.uint8))

            # Resize the heatmap to the target dimensions
            raw_hm_resized = raw_hm_image.resize((width, height))

            # Inout conversion and modulation
            inout = inout.cpu().detach().numpy()
            inout = 1 / (1 + np.exp(-inout))
            inout = (1 - inout) * 255

            # Convert inout to a PIL image for manipulation
            inout_image = Image.fromarray(inout.astype(np.uint8))

            # Subtract inout from the resized heatmap
            norm_map = np.array(raw_hm_resized) - np.array(inout_image)
